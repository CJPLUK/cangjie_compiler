    ...
    std::vector<Ptr<Ty>>& included)
{
    ...
    if (maybeRtp) {
        ...
        if (!Check(ctx, rtp.GetTy(), rtp.pattern.get())) {
            return false;
        }
    }
    ...
    const TryExpr& te, ResumptionTypePattern& rtp)
{
    ...
    auto prTys = promotion.Promote(*type->GetTy(), *resumption->GetTy());
    if (type->GetTy()->IsNothing() || prTys.empty() || !Ty::IsTyCorrect(type->GetTy())) {
    ...
    if (auto tryLambdaTy = DynamicCast<FuncTy*>(te.tryLambda->GetTy()); tryLambdaTy) {
